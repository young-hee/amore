	...
	var SnsShare = function ( $target, pluginName ) {
		...
		function copyUrl ( $el ) {
			...
			if ( typeof document.execCommand === 'function' ) {
				var $input = $( '<input type="text" value="' + Origin.href + '" style="position:absolute">' );
				$el.parent().append( $input );
				$input.select();
				document.execCommand( 'Copy' );
				$input.remove();
				$el.focus();
				AP.modal.alert( '주소가 복사되었습니다.<br>게시판이나 메신저 창에서 Ctrl+V를 눌러보세요.' ).addListener( 'modal-close', function (e) {
					_modal.close();
					dispatch( 'call', 'url' );
				});
			}
