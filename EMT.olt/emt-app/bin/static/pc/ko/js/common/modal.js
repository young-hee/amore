...
;(function ( $, common ) {
    ...
    var modal = {
			...
			var opt = options || {},
				...
				api = AP.api.getAgreeTerms( null, {
					...
					if( data ) {
						...
						if ( terms ) {
							modal.getElement().find( '.layer_cont' ).html( '<pre>' + terms.termsBodyText + '</pre>' );
							modal.resetPosition();
						}
					}
				});
         ...
         * 전체 모달 포지션 재설정
         */
        resetPosition: function ( isReset ) {
            if ( this.isOpen() ) {
                $( '.layer_popup.js_open' ).each( function ( idx, el ) {
                    $( el ).triggerHandler( 'resetPosition', [isReset] );
                });
            }
        }
    };
    ...
    var ModalCore = $B.Class.extend({
        ...
        _draw: function ( templateKey ) {
            ...
            this._$body.append( this._$dimmed );
        ...
        _addEvents: function () {
            ...
            this._$modal.on( 'resetPosition', function (e, isReset ) {
                this._setPosition( !isReset );
                //this._setDimmedHeight();
            }.bind(this));
        ...
        _setPosition: function ( isRePosition ) {
            ...
            if ( this._options.middlePosition ) {
				var iconH = Math.abs( parseFloat(this._$iconClose.css('top')) || 0 ),
					winHeight = this._$win.height(),
					docHeight = this._$heightTarget.outerHeight(),
                    popHeight = this._$pop.outerHeight(),
                    posY = ( winHeight * 0.5 ) - ( popHeight * 0.5 ) + this._scrollTop,
                    positionTime = new Date().getTime();

                if ( posY < iconH + this._scrollTop ) posY = iconH + this._scrollTop;
				if ( posY + popHeight > docHeight ) {
					posY = posY - ( (posY + popHeight) - docHeight );
                ...
                if ( isRePosition && (this._rePositionTime && positionTime - this._rePositionTime > 50) ) {
					...
					this._$pop.removeClass( 'transition_top' );
				}
