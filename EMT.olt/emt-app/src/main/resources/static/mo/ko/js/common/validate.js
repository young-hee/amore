...
;(function( $ ) {
	...
	//email
	$.validator.addMethod( 'email', $.validator.methods.email, '이메일주소를 정확히 입력해 주세요.' );
	//음수, 양수, 소수
	...
	*/
	...
	/**
	 * 14세 이상만 신청가능 
	 * 주민등록번호가 없고, 생년월일이 앞자리가 없어서.. 40년생은 2000년으로 임의로 계산함 .. .
	 * 수정이 필요하면 생년월일을 full로 변경해야함
	 */
	...
	* -> 아이디 기준에 충족하지 않게 입력한 경우
	...
	$.validator.setDefaults({
		...
		highlight: function( element, errorClass, validClass ) {
			...
			if ( element.type === 'radio' || element.type === 'checkbox' || $el.is('.ui_date_picker, .input_file_name') ) {
				$el.parent().addClass(errorClass).removeClass(validClass);
			} else {
				$el.parent().parent( '.input_group' ).addClass( errorClass ).removeClass( validClass );
		...
		unhighlight: function( element, errorClass, validClass ) {
			...
			if ( element.type === 'radio' || element.type === 'checkbox' || $el.is('.ui_date_picker, .input_file_name') ) {
				...
				$el.parent().parent( '.input_group' ).removeClass( errorClass ).addClass( validClass );
