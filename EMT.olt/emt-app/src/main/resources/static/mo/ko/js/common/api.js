...
;(function ( $ ) {
	...
	var Setting = {
		...
		searchReviewList: { path: '/common/searchReviewList', data: {
			 toSearchFor			: ''
			,toBeExcluded			: null
			,prodReviewTypeCodes 	: ''//상품평유형코드
			,prodReviewSort 		: ''//상품평유형코드
			,scopes  				: ''//상품평유형코드
			,offset					: 0
			,limit 					: 3
		}},
		...
		//리뷰 검색
		searchReview : {path: '/common/searchReview', data: {
			 limit : 10,
			 prefix : null
		}},
		...
		cashReceiptIssueRequest: { path: '/my/api/cashReceiptIssueRequest', method: 'POST', contentType : false, processData : false},

		ordSelect: { path: '/my/api/{ordSn}/{type}/ordSelect', method: 'POST', contentType: 'application/json' },
		...
		onlineProdUnitVariationProds: { path: '/my/api/{ordHistProdSn}/onlineProdUnitVariationProds', method: 'GET' },
		...
		regularEventTermsAgree : { path:'/display/regularEventTermsAgree', method: 'POST', contentType : false, processData : false, data: {
		...
		*/
		...
		/***
		 * 이벤트 상세 내용
		 * request : parameter : Long foNoticeSn
		 * response : 
		 */
		winnerNoticeDetail : { path:'/display/winnerNoticeDetail', method: 'GET', data: {
			foNoticeSn : null // 공지 일련번호 
			}
		},
		...
		// 상품목록 추출
		getCart: { path: '/cart/getCart', method: 'GET'},
        ...
        //매장찾기
		//FO매장행사코드
		//foStoreEventCode : '' (Color Finder, Color Factory, Artist Service)
		...
		/**
		 * App전용 App version check ***********************************************
		 * @param OsType (Android or iOS) 
		 * 
		 * @return  hashMap {
		 *           appVersion : 앱버전  
		 * 	         mandatory : 업데이트 필수 여부 (Y,N)} 
		 * 	ex) {"appVersion": "string", "mandatory": "Y" }
		 */
		getAppVersion: { path:'/application/getAppVersion', method: 'GET', data: {
			OsType: null // 기기os버전 (필수)
		 ...
		 * 			 {"appVer": "string", // 앱버전
		 * 			  "appUid": "string", // 앱UID
    	 *			  "modelName": "string", // 기기모델명
    	 *            "osVer": "string", // 기기 os 버전 
  		 *			  "token": "string", // 기기 토큰 
  		 *            "agreeYn": "string", // 푸쉬 동의 여부
  		 *			  "memberYn": "string",// 회원 여부
  		 *            "memberSn":'int' // 회원 일련 번호}
		 * 
		 * @response hashMap {"return : true"} 
		 */
		saveAppInstall: { path:'/application/saveAppInstall', method: 'POST', contentType:false, processData: false },
		
		/**
		 * App전용 앱 설치 내용 호출 ***********************************************
		 * @param osType (Android or iOS) , token (앱 토큰)
		 * @response {"return : true"} json 
		 */
		
		getAppInstall: { path:'/application/getAppInstall', method: 'GET', data: {
			OsType: null, // 기기os버전 (필수)
			token: null // 앱 토큰 (필수)
		}},
		...
		updateAppAgree: { path:'/application/updateAppAgree', method: 'PUT', data: {
			token: null // 앱 토큰 (필수)
		}},
		...
		updateAppLogin: { path:'/application/updateAppLogin', method: 'PUT', data: {
			token: null // 앱 토큰 (필수)
		}},
		
		/**
		 * App전용 비 회원 앱 접속 업데이트  ***********************************************
		 * @param token (앱 토큰)
		 * @response {"return : true"} json 
		 */
		
		updateAppConnect: { path:'/application/updateAppConnect', method: 'PUT', data: {
			token: null // 앱 토큰 (필수)
		}},
		
		/**
		 * App전용 토큰으로 푸쉬발송성공여부  ***********************************************
		 * @param token (앱 토큰) , success (성공여부)
		 * @response {"return : true"} json 
		 */
		
		updateAppSendSuccess: { path:'/application/updateAppSendSuccess', method: 'PUT', data: {
			token: null, // 앱 토큰 (필수)
			success: null 
		}},
		 ...
		 * @return hashMap
		 */
		colorPickingPolicy: { path:'/common/colorPickingPolicy', method: 'GET', data: {
			  termCode: null // 약관종류 (필수)
		}},
		
		/**
		 * 컬러픽킹 상품 제안 ***********************************************
		 * @param int : red, int: green, int :blue , int : peal , int: glossy  
		 * @return hashMap
		 */
		getColorPickProd: { path:'/application/getColorPickProd', method: 'GET', data: {
			  red: null , // 
			  green : null , //
			  blue : null , // 
			  peal : null , //
